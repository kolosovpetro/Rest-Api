...
namespace Api.Tests.MoqTests
{
    ...
    public class MoviesServicesTest
    {
        public async Task ServiceTest()
        {
            ...
            var mockServices = new Mock<MoviesService>(mockRepo.Object, mapper);
            mockServices.Setup(getAllMoviesAsync => getAllMoviesAsync.GetAllMoviesAsync())
                .Returns(MappedMovies(allMovies, mapper));
        ...
        private Task<IEnumerable<MoviesReadDto>> MappedMovies(IEnumerable<Movies> collection, IMapperBase mapper)
        {
